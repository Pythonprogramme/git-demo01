...
def index(request):
    return render(request, 'order/service.html')
...
def list_values(request):
    ...
    objs = list(Order.objects.filter(Q(sj_name__icontains=q_str)|Q(product_name__icontains=q_str)|
                                     Q(jz_name__icontains=q_str)|Q(service_name__icontains=q_str)|Q(shipping_status__icontains=q_str)).values('order_number','sj_name','sj_mobile','order_date',
